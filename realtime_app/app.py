...
async def sse_publish(payload: Dict[str, Any]):
    """Fan-out payload to all connected SSE clients and log success."""
    total_before = len(sse_clients)
    dead = []
    delivered = 0
    for q in list(sse_clients):
        try:
            ...
            print(f"[SSE][ERROR] queue put failed: {e}")
            dead.append(q)
    for q in dead:
        sse_clients.discard(q)

    print(f"[SSE] queued event to {delivered}/{total_before} client(s): {payload}")

@app.get("/sse")
async def sse_endpoint(request: Request):
    """
    SSE endpoint that pushes 'incident' events.
    Each event's data is: {"lat": <float>, "lon": <float>, "accident_type": <str>}
    """
    q: asyncio.Queue = asyncio.Queue()
    sse_clients.add(q)
    print(f"[SSE] client connected; active={len(sse_clients)}")

    async def event_generator():
        # Ask client to retry after 5s if connection drops
        yield "retry: 5000\n\n"
        try:
            while True:
                if await request.is_disconnected():
                    print("[SSE] client disconnected (HTTP disconnect detected)")
                    break
                try:
                    data = await asyncio.wait_for(q.get(), timeout=20)
                    # Write event
                    yield "event: incident\n"
                    yield f"data: {json.dumps(data, default=_json_default)}\n\n"
                    print(f"[SSE] sent event to one client: {data}")
                except asyncio.TimeoutError:
                    # Keep connection alive (comment shows in devtools but ignored by clients)
                    yield ": keep-alive\n\n"
        finally:
            sse_clients.discard(q)
            print(f"[SSE] client removed; active={len(sse_clients)}")
        ...
        if score >= 0.7:
            try:
                ...
                try:
                    ...
                    # Publish SSE event to frontend with minimal payload
                    try:
                        payload = {
                            "lat": confirmed.lat,
                            "lon": confirmed.lon,
                            "accident_type": confirmed.hazard
                        }
                        await sse_publish(payload)
                        print(f"[SSE] publish requested successfully: {payload}")
                    except Exception as e:
                        print(f"[SSE][ERROR] {e}")
