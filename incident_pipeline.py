...
- Live inputs:
  ...
  * USGS Earthquakes (nearest recent event & Shake-like impact bands)
- Dispatcher picks the right predictive module based on accident_type.
- NEW: Agent can ingest handoff files written by app.py:
  * --handoff <path> : process a single HANDOFF_*.json
  * --watch          : watch agent_outbox/ for new HANDOFF_*.json and process continuously
  * Results saved to agent_results/PLAN_<handoff_id>.json
"""
...
def run_incident(lat: float, lon: float, accident_type: str) -> Optional[Dict[str, Any]]:
    """
    Runs the full agent workflow and prints to console. Returns the final JSON (dict) if parsing succeeds,
    otherwise None (but still prints the assistant message content).
    """
    print("=" * 80)
    print(f"DISASTER RESPONSE SYSTEM - INCIDENT REPORT (Letta)")
    print(f"Location: {lat}, {lon}")
...
Output ONLY JSON as specified earlier."""
    ...
    for m in final_resp.messages:
        if m.message_type == "assistant_message":
            print(m.content)
            # Try to parse the JSON (PlannerAgent is instructed to return JSON only)
            try:
                final_json = json.loads(m.content)
            except Exception:
                pass
    print("=" * 80)
    return final_json
...
# =====================================================================================
#                            HANDOFF CONSUMPTION HELPERS
# =====================================================================================

HANDOFF_DIR = Path(os.getenv("AGENT_OUTBOX_DIR", "agent_outbox")).resolve()
RESULTS_DIR = Path(os.getenv("AGENT_RESULTS_DIR", "agent_results")).resolve()
...
def run_incident_from_handoff(handoff_path: Path) -> Optional[Path]:
    """
    Read a HANDOFF_*.json, run the agent workflow, and write PLAN_<id>.json to agent_results/.
    Returns the output file path, or None on failure.
    """
    try:
        with handoff_path.open("r", encoding="utf-8") as f:
            handoff = json.load(f)
        inc = handoff.get("incident", {})
        hid = handoff.get("id") or handoff_path.stem.replace("HANDOFF_", "")
        lat = float(inc["lat"]); lon = float(inc["lon"]); atype = str(inc["accident_type"])
    except Exception as e:
        print(f"[Handoff][ERROR] Reading {handoff_path.name}: {e}")
        return None

    print(f"[Handoff] Processing {handoff_path.name} -> ({lat},{lon}) type='{atype}'")

    plan = run_incident(lat=lat, lon=lon, accident_type=atype)
    if plan is None:
        print(f"[Handoff][WARN] Planner output was not valid JSON; skipping file write.")
        return None

    out_path = RESULTS_DIR / f"PLAN_{hid}.json"
    try:
        _atomic_write_json(out_path, plan)
        print(f"[Handoff] Wrote {out_path}")
        return out_path
    except Exception as e:
        print(f"[Handoff][ERROR] Writing result: {e}")
        return None

def watch_handoff_folder(interval: int = 10):
    """
    Continuously watch HANDOFF_DIR for new HANDOFF_*.json files and process them once.
    """
    print(f"[Watcher] Watching {HANDOFF_DIR} every {interval}s; results -> {RESULTS_DIR}")
    seen: set[str] = set()
    while True:
        try:
            for f in HANDOFF_DIR.glob("HANDOFF_*.json"):
                key = str(f.resolve())
                if key in seen:
                    continue
                # Basic sanity: non-empty file (avoid partial writes if app uses atomic rename)
                try:
                    if f.stat().st_size == 0:
                        continue
                except FileNotFoundError:
                    continue
                run_incident_from_handoff(f)
                seen.add(key)
        except Exception as e:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Disaster-response multi-agent runner")
    parser.add_argument("--handoff", type=str, help="Path to HANDOFF_*.json to process")
    parser.add_argument("--watch", action="store_true", help="Watch agent_outbox/ for new handoff files")
    parser.add_argument("--lat", type=float, help="Latitude for manual run")
    parser.add_argument("--lon", type=float, help="Longitude for manual run")
    parser.add_argument("--type", type=str, help="Accident type for manual run (e.g., 'urban fire')")
    args = parser.parse_args()

    if args.handoff:
        run_incident_from_handoff(Path(args.handoff))

    elif args.watch:
        watch_handoff_folder()

    elif args.lat is not None and args.lon is not None and args.type:
        # Manual one-off run with explicit params
        run_incident(lat=args.lat, lon=args.lon, accident_type=args.type)
